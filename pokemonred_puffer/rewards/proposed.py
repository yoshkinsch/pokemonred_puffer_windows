...
class ProposedRewardEnv(RedGymEnv):
...
class TeachCutReplicationEnv(ProposedRewardEnv):
    def get_game_state_reward(self):
        return {
...
class TeachCutReplicationEnvFork(ProposedRewardEnv):
    def get_game_state_reward(self):
        return {
            "event": self.reward_config["event"] * self.update_max_event_rew(),
            "met_bill": self.reward_config["bill_saved"]
...
class CutWithObjectRewardsEnv(ProposedRewardEnv):
    def get_game_state_reward(self):
        return {
            "event": self.reward_config["event"] * self.update_max_event_rew(),
            "met_bill": self.reward_config["bill_saved"]
...
class CutWithObjectRewardRequiredEventsEnv(ProposedRewardEnv):
    def get_game_state_reward(self):
...
class ObjectRewardRequiredEventsEnvTilesetExploration(ProposedRewardEnv):
    def get_game_state_reward(self):
        _, wBagItems = self.pyboy.symbol_lookup("wBagItems")
        bag = np.array(self.pyboy.memory[wBagItems : wBagItems + 40], dtype=np.uint8)
        numBagItems = self.read_m("wNumBagItems")
...
class ObjectRewardRequiredEventsMapIds(ProposedRewardEnv):
...
class ProposedObjectRewardRequiredEventsMapIdsFieldMoves(ObjectRewardRequiredEventsMapIds):
    def get_game_state_reward(self) -> dict[str, float]:
